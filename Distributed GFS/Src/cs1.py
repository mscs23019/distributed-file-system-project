...
DATA_DIR = os.path.expanduser("~/gfs_root/cs1")
...
if __name__ == "__main__":
    print(f"GFSChunkServer is Running on port {config.CHUNK_SERVERS[0]['port']}!")
    if not os.path.isdir(DATA_DIR):
        os.makedirs(DATA_DIR)
    t = ThreadedServer(GFSChunkService, port=config.CHUNK_SERVERS[0]['port'])
    t.start()
